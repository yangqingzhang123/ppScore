...
//int Http_Server::svc(size_t idx)
int Http_Server::svc()
{
	int fd,new_fd;
	...
	while (!_stop_task)
	{
		...
		if (fd == _socket_server_listen)
		{
			...
			while ((new_fd = accept(fd, &client_addr, &s_len)) >= 0)
			{
				...
				else
				{
                    ...
                    _INFO("Client ip:%s:%d", inet_ntoa(sin.sin_addr), sin.sin_port);
					_now_connections++;
					pthread_mutex_unlock(&_connections_mutex);
...
SocketHandle *Http_Server::create_handle(int fd, int flag)
{
...
{
	SocketHandle * hdle = create_handle(fd, LISTEN_PORT_1);
	_ERROR("[TEST]recv %d\n",fd);
	assert(hdle);
	hdle->acquire();
...
int Http_Server::retrieve_worker(Worker * worker)
{
	...
	worker->handle->set_flag(worker->handle->get_flag() | ERROR_SOCKET);
