...
//modified by lidw
//date :2015-12-08
//说明:原时间处理库接口清晰易用，但却不能用于多线程编程，否则会使多线程蜕化为单线程, 追踪原因是因为该库使用了tzset() 和 mktime()等两个时间处理函数，现修改之
//      (1)对于tzset()全部注释掉，带来的影响有两点:a.不能感受系统时区文件 "/etc/localtime"的变化(除非开发人员手工操作，此文件一般不会变化)
//                                                 b.调用setenv 或者 unsetenv对环境变量TZ进行改变后，需要主动显式调用tzset()
//      (2)使用boost中的date_time库中函数代替mktime(),对使用无影响(需要安装 boost-static)
                   ...
                   int dwTimezoneSeconds)
{
    ...
    time_t t = (boost::posix_time::ptime_from_tm(oTime)-boost::posix_time::ptime(boost::gregorian::date(1970,1,1))).total_seconds();
    m_Time.m_sec = t;
    m_Time.m_usec = dwMilliSecond * 1000 + dwMicroSecond;
    m_Time.Normalize();
}
    ...
    const DateTimeFormatInfo& formatInfo /*= DateTimeFormatInfo::zh_CNInfo*/)
{
    ...
    if(!IsValidDate(date_info.year,date_info.month,date_info.day_of_month))
        return false;

    /*
     * zhanghongtao 2016-05-31 
     */
    if (date_info.year < 1970)
        return false;
